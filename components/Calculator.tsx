  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        ...
        {/* Action Button that fills the remaining gap with deep shadow */}
        <div className="flex-1 flex min-h-[120px]">
          <Button 
            onClick={saveProject} 
            className="w-full h-full text-lg shadow-2xl shadow-blue-500/40 hover:shadow-blue-500/60 transition-all duration-300 transform active:scale-[0.98]" 
            disabled={saving}
          >
            <div className="flex flex-col items-center gap-3">
              {saving ? <Loader2 className="animate-spin" size={32} /> : <Save size={32} className="drop-shadow-sm" />} 
              <span className="font-black tracking-tight">{saving ? 'Salvando...' : 'Salvar Orçamento'}</span>
            </div>
          </Button>
        </div>
      </div>
